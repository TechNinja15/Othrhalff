...
import { EmojiClickData } from 'emoji-picker-react'; // If you use the picker elsewhere, keep import
...
const REACTIONS = ['❤️', '😂', '🔥', '😮', '😢', '👀'];
const POSTS_PER_PAGE = 10; // Load 10 at a time
...
export const Confessions: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel('confessions-realtime')
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'confessions' }, (payload) => {
                ...
                if (p.user_id === currentUser.id) return; // Ignore own posts (handled optimistically)
    ...
    }, [currentUser, sortType]); // Re-fetch if sort changes

    // 2. Fetch Logic with Pagination
    const fetchConfessions = async (pageIndex: number, reset = false) => {
        ...
        // Sort logic in DB to avoid fetching all
        ...
        // Note: 'popular' and 'discussed' are hard to sort DB-side without computed columns. 
        // For performance, we stick to time-based sorting for the main feed, or client-sort if unavoidable,
        // but client-sorting breaks pagination. Ideally, default to Newest.
        ...
        // Fetch user votes for these specific posts
        ...
        const formatted: Confession[] = posts.map((p: any) => {
            ...
            return {
                ...
                comments: p.confession_comments || [],
                university: p.university,
                type: p.type as 'text' | 'poll',
        ...
        if (reset) {
            ...
            // Filter duplicates just in case realtime added one that we just fetched
            setConfessions(prev => {
                const existingIds = new Set(prev.map(c => c.id));
                const uniqueNew = formatted.filter(c => !existingIds.has(c.id));
    ...
    const handlePost = async () => {
        ...
        try {
            ...
            // Re-fetch only the first page to see the new post immediately (or rely on realtime)
            setPage(0);
            fetchConfessions(0, true);
    ...
    const handlePollVote = async (confessionId: string, optionId: string) => {
        ...
        try {
            await supabase.from('poll_votes').insert({ confession_id: confessionId, option_id: optionId, user_id: currentUser.id });
    ...
    const handleReaction = async (id: string, emoji: string) => {
        ...
        try {
            if (previousReaction === emoji) {
                await supabase.from('confession_reactions').delete().eq('confession_id', id).eq('user_id', currentUser.id);
            } else {
                analytics.confessionReact(emoji);
                await supabase.from('confession_reactions').upsert({ confession_id: id, user_id: currentUser.id, emoji }, { onConflict: 'confession_id,user_id' });
            }
        ...
        <div className="h-full w-full bg-transparent text-white flex flex-col relative overflow-hidden font-sans">
            ...
            <div className="flex-none p-4 border-b border-gray-800/50 bg-black/20 backdrop-blur-md z-40 sticky top-0 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold uppercase tracking-tight">Campus Confessions</h1>
                        <p className="text-xs text-gray-500 font-mono">Amity University</p>
                    </div>
                </div>

                {/* Sort Menu - Simplified: Only enable if desired, but careful with pagination */}
                <div className="relative">
                    <button
                        onClick={() => setShowSortMenu(!showSortMenu)}
                        className={`p-2 rounded-full transition-colors ${showSortMenu ? 'bg-white text-black' : 'bg-gray-900 text-gray-400'}`}
                    >
                            ...
                            <div className="absolute right-0 top-12 w-48 bg-gray-900 border border-gray-800 rounded-lg shadow-xl z-20">
                                {['newest', 'oldest'].map((type) => (
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-2 text-sm text-gray-400 hover:bg-gray-800 capitalize"
                                    >
                                        {type}
                                    </button>
                                ))}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4 pb-32 md:pb-24 relative z-10">
                {confessions.length === 0 && !isLoadingMore ? (
                    ...
                    confessions.map(conf => (
                        <div key={conf.id} className={`bg-gray-900/30 backdrop-blur-md border rounded-xl p-4 ${conf.id === '46c46dcc-ad75-487d-b5a4-70b03081c222' ? 'border-neon/50' : 'border-gray-800/50'}`}>
                            {/* ... (Identical Render Logic for Card) ... */}
                                        ...
                                        return (
                                            <button key={option.id} onClick={() => handlePollVote(conf.id, option.id)} disabled={!!conf.userVote} className="w-full relative h-9 rounded border border-gray-800 overflow-hidden">
                                                ...
                                                <div className="absolute inset-0 flex items-center justify-between px-3 z-10">
                                                    <span className="text-xs font-medium text-gray-400">{option.text}</span>
                                                    {conf.userVote && <span className="text-[10px] font-bold text-gray-500">{pct}%</span>}
                                                </div>
                                            </button>
                                        );
                                    })}
                            ...
                            <div className="flex flex-col gap-2 border-t border-gray-900 pt-3">
                                {conf.reactions && Object.values(conf.reactions).some(v => v > 0) && (
                                    <div className="flex flex-wrap gap-1.5 mb-2">
                                        {Object.entries(conf.reactions).map(([e, c]) => c > 0 && (
                                            <span key={e} className="inline-flex items-center gap-1 bg-gray-900 text-[10px] px-2 py-0.5 rounded-full text-gray-400 border border-gray-800">
                                                {e} <b>{c}</b>
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="flex items-center gap-3">
                                    <button onClick={(e) => handleReactionClick(e, conf.id)} className="flex items-center gap-2 text-gray-500 hover:text-white text-xs px-2 py-1 rounded-md hover:bg-gray-900">
                                        <SmilePlus className="w-4 h-4" /> React
                                    </button>
                                    <button onClick={() => toggleComments(conf.id)} className="flex items-center gap-2 text-gray-500 hover:text-blue-400 text-xs px-2 py-1 rounded-md hover:bg-gray-900">
                            ...
                            {expandedComments[conf.id] && (
                                <div className="mt-3 pt-3 border-t border-gray-900">
                                    <div className="space-y-2 mb-3 max-h-96 overflow-y-auto">
                                        {conf.comments?.map(c => (
                                            <div key={c.id} className="bg-gray-900/40 p-2 rounded-lg">
                                                <div className="flex justify-between mb-1"><span className="text-[10px] font-bold text-gray-500">{c.userId}</span></div>
                                                <p className="text-xs text-gray-300">{c.text}</p>
                                            </div>
                                        ))}
                                    </div>
                                    <div className="flex gap-2">
                                        <input className="flex-1 bg-black border border-gray-800 rounded-lg px-3 py-2 text-xs text-white"
                                            placeholder="Comment..."
                                        ...
                                        />
                                        <button onClick={() => handleCommentSubmit(conf.id)} className="p-2 bg-gray-800 text-white rounded-lg"><Send className="w-3.5 h-3.5" /></button>
                                    </div>
                                </div>
                            )}
                        </div>
            ...
            {/* Fixed Input Area (Identical to before) */}
            ...
            {activeReactionMenu && (
                <>
                    ...
                    <div className="fixed z-50 bg-black/80 backdrop-blur-md border border-gray-800 rounded-2xl p-2" style={menuPosition ? { top: menuPosition.top, left: menuPosition.left } : {}}>
                        <div className="flex gap-1">{REACTIONS.map(emoji => <button key={emoji} onClick={() => handleReaction(activeReactionMenu, emoji)} className="text-2xl hover:scale-125 transition-transform p-2">{emoji}</button>)}</div>
                    </div>
                </>
            )}
